...
There are a few options in terms of interacting with the nodes – the modified Corda GUI, altering the simulation to call different methods provided, making API calls (using an app such as Postman), or through the provided Web UI. To run any of these, first make sure you have run the NodeDriver.kt file as stated above. The recommended method is using the CordaGUI.
